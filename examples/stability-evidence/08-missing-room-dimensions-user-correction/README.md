# Case 08: стёртые размеры помещения и исправление пользователя

Фактический запуск `audit_med2_20260719` от 19 июля 2026 года, версия скилла `0.7.7`. На плане намеренно стёрты линейные размеры помещения «Ресепшен». Ожидаемый результат — Vision не должен восстанавливать невидимые значения по масштабу или типовым предположениям; workflow обязан запросить недостающие размеры, принять явную пользовательскую correction и выполнить расчёты только по новой подтверждённой revision.
...
4. `confirm_geometry(... confirmed=false)` применил ограниченные corrections к `room_002.width_m`, `room_002.length_m` и высоте всех помещений и создал revision 2.
5. Deterministic geometry пересчитала зависимые поля «Ресепшен»: площадь `38,64 м²` и периметр `26,8 м`.
6. Revision 2 была снова показана и отдельно подтверждена пользователем.
7. После подтверждения скилл получил каталог MCP, выполнил deterministic audit и сформировал отчёт.
...
В [`geometry.json`](output/geometry.json) длина и ширина имеют `source_type: user_correction`, а площадь и периметр — `source_type: derived_from_explicit_dimensions`. Полные формулы и последующие quantity checks сохранены в [`calculation_trace.json`](output/calculation_trace.json).
...
| Geometry corrections | 1 событие, 3 correction items |
| Помещений / площадь пола | 5 / 164,36 м² |
| Уникальных дверей / окон | 6 / 5 |
| Missing fields / conflicts после correction | 0 / 0 |
| Строк сметы | 35 |
...
Скилл сохраняет происхождение каждого значения и не смешивает Vision с пользовательскими данными. Невидимые размеры не были угаданы; введённые пользователем `9,2 × 4,2 м` стали явным источником revision 2, а все зависимые величины и проверки были рассчитаны Python-кодом уже после отдельного подтверждения.

...
flowchart TD
    ...
    K -->|Аудит готовой сметы| M{Все соответствия точные?}
    M -->|Да| MA[Детерминированное сопоставление]
    M -->|Нет| MB[Субагент Mapping]
    MA --> MC[Проверить и сохранить Mapping]
    MB --> MC
    MC --> N[Детерминированный аудит]
...
После сохранения каталога Python сначала пытается сопоставить помещения и каждую строку сметы по точным однозначным названиям и совместимым единицам. Если полное точное сопоставление невозможно, запускается изолированный Mapping-субагент для смысловых соответствий. И детерминированный результат, и ответ субагента проходят одинаковую проверку схемы перед расчётами; сам аудит в обоих случаях выполняется детерминированным Python-кодом.

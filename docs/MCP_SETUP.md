...
Так цены можно обновлять независимо от кода аудита, а скилл получает только явный, сохранённый и валидируемый каталог. Если MCP недоступен, workflow завершается ошибкой `price_catalog_unavailable` и не подставляет цены из памяти модели.

...
Дополнительный набор [`examples/stability-evidence`](examples/stability-evidence/README.md) содержит десять независимых реальных запусков в едином формате «вход → полный output». Сценарии версии `0.7.7` охватывают намеренные ошибки в смете, намеренно подтверждённую неверную геометрию, корректную смету без ложных findings, plan-only workflow, отсутствующую дверную работу, semantic Mapping, безопасную остановку при недоступном MCP, расчёт по явно введённым размерам и независимое распознавание помещения, удалённого из XLSX. Во всех семи завершённых аудитах с исходной XLSX coverage количества и цены составил 100%; два plan-only кейса сформировали расчётные XLSX, а MCP-кейс намеренно остановился до начала аудита.

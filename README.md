...
Дополнительный набор [`examples/stability-evidence`](examples/stability-evidence/README.md) собирает десять независимых реальных запусков в едином формате «вход → полный output». Уже включены первые семь сценариев версии `0.7.7`: намеренные ошибки в смете, намеренно подтверждённая неверная геометрия, корректная смета без ложных findings, plan-only запуск с обязательным подтверждением geometry, отсутствующая дверная работа, semantic Mapping и безопасная остановка при недоступном MCP. Во всех пяти завершённых аудитах с исходной XLSX coverage количества и цены составил 100%; MCP-кейс намеренно остановился до начала аудита.

...
Совместимость и описанные в репозитории сценарии проверены на Ouroboros `6.61.4`.
Работа с другими версиями отдельно не подтверждалась.
...
>
> **Два реальных сценария с короткими видео:** действия пользователя, вызовы инструментов,
...
В [`examples/medical-office`](examples/medical-office/README.md) сохранён обезличенный сквозной запуск от 17–18 июля 2026 года:

- план медицинского офиса и XLSX-смета на 35 строк;
- две фотографии объекта;
- подтверждённая геометрия, ревизия 2;
- 100% полнота проверки объёмов и цен;
...
Дополнительный набор [`examples/stability-evidence`](examples/stability-evidence/README.md) содержит десять независимых реальных запусков в едином формате «входные данные → полный набор результатов». Сценарии версии `0.7.7` охватывают намеренные ошибки в смете, подтверждение пользователем ошибочно распознанной геометрии, корректную смету без ложных расхождений, работу только с планом, пропущенную в смете установку двери, смысловое сопоставление, безопасную остановку при недоступном MCP, расчёт по явно введённым размерам и независимое распознавание помещения, отсутствующего в XLSX. Во всех семи завершённых проверках исходной XLSX полнота проверки объёмов и цен составила 100%; два сценария только с планом сформировали расчётные XLSX-сметы, а сценарий с недоступным MCP намеренно остановился до начала проверки.

В сценарии проверки исходной сметы контрольная XLSX создаётся отдельным финальным действием: после отчёта скилл предлагает сформировать независимую смету по подтверждённому плану и каталогу MCP. Файл создаётся только после явного согласия пользователя и позволяет сопоставить исходную смету с независимо рассчитанным вариантом.
